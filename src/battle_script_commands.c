...
{
    // sExperienceScalingFactors[i] = floor(i^2.5 / 4). Indexed by
    // (2*faintedLevel+10) and (faintedLevel+expGetterLevel+10) in
    // ApplyExperienceMultipliers, so it must cover 2*MAX_LEVEL+10.
    0,
    0,
    1,
    3,
...
STATIC_ASSERT(ARRAY_COUNT(sExperienceScalingFactors) > 2 * MAX_LEVEL + 10, sExperienceScalingFactorsCoversMaxLevel);
