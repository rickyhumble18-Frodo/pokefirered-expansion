...
static void UpdateLvlInHealthbox(u8 healthboxSpriteId, u8 lvl)
{
    ...
    if (IsOnPlayerSide(battler))
    {
        ...
        AddSpriteTextPrinterParameterized6(spriteId, FONT_SMALL, 31 - width, 3, 0, 0, sHealthBoxTextColor, 0, text);
    }
    else
    {
        FillSpriteRectColor(spriteId, 0, 5, 24, 11, HEALTHBOX_BG_INDEX);
        AddSpriteTextPrinterParameterized6(spriteId, FONT_SMALL, 23 - width, 3, 0, 0, sHealthBoxTextColor, 0, text);
    }
}
...
static void PrintHpOnHealthbox(u32 spriteId, s16 currHp, s16 maxHp, u32 bgColor, u8 yOffset)
{
    ...
    *txtPtr++ = CHAR_SLASH;
    ...
    if (width < 31)
        AddSpriteTextPrinterParameterized6(spriteId2, HP_FONT, 31 - width, yOffset + 5, 0, 0, sHealthBoxTextColor, 0, text);
    else
        AddSpriteTextPrinterParameterized6(spriteId, HP_FONT, 63 - (width - 32), yOffset + 5, 0, 0, sHealthBoxTextColor, 0, text);
    ...
    for (enum BattlerId i = 0; i < gBattlersCount; i++)
    {
         ...
         && (GetBattlerCoordsIndex(i) != BATTLE_COORDS_SINGLES || !IsOnPlayerSide(i)))
        {
            ...
            if (IsOnPlayerSide(i))
            {
                ...
                {
                    FillSpriteRectColor(gHealthboxSpriteIds[i], 32, 16, 32, 9, HEALTHBOX_BG_INDEX);
                    FillSpriteRectColor(gSprites[gHealthboxSpriteIds[i]].oam.affineParam, 0, 16, 32, 9, HEALTHBOX_BG_INDEX);
...
static void UpdateLeftNoOfBallsTextOnHealthbox(u8 healthboxSpriteId)
{
    ...
    FillSpriteRectColor(healthboxSpriteId, 53, 19, 31, 12, HEALTHBOX_BG_INDEX);
    AddSpriteTextPrinterParameterized6(healthboxSpriteId, FONT_SMALL, 53, 19, 0, 0, sHealthBoxTextColor, 0, text);
...
void UpdateHealthboxAttribute(u8 healthboxSpriteId, struct Pokemon *mon, u8 elementId)
{
    ...
    if (IsOnPlayerSide(battler))
    {
        ...
        if (!isDoubles && (elementId == HEALTHBOX_EXP_BAR || elementId == HEALTHBOX_ALL))
        {
            ...
            else // at the cap, show an empty bar (indexing level + 1 would read past the table)
            {
                currExpBarValue = 0;
                maxExpBarValue = 1;
            }
